document.addEventListener('DOMContentLoaded', function() {
    ...
    // Event Listeners
    courseSearch.addEventListener('input', debounce(function(e) {
    ...
    // Filter checkboxes
    document.querySelectorAll('input[id^="level"], input[id^="duration"]').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            const type = this.id.startsWith('level') ? 'levels' : 'durations';
            if (this.checked) {
                currentState[type] = [...currentState[type], this.value];
            } else {
                currentState[type] = currentState[type].filter(val => val !== this.value);
            }
            updateUI();
        });
    });
    ...
    function filterAndSortCourses() {
        ...
        let filtered = courses.filter(course => {
            const matchesSearch = course.name.toLowerCase().includes(currentState.searchTerm);
            const matchesLevel = currentState.levels.length === 0 || currentState.levels.includes(course.level);
            const matchesDuration = currentState.durations.length === 0 || currentState.durations.includes(course.duration);
    ...
    function updateActiveFilters() {
        ...
        currentState.durations.forEach(duration => {
            addFilterChip(`Duration: ${duration} hours`, () => {
                ...
                if (checkbox) checkbox.checked = false;
                currentState.durations = currentState.durations.filter(d => d !== duration);
                updateUI();
            });
        });
    }
    ...
    function addFilterChip(text, removeCallback) {
        ...
        chip.className = 'badge rounded-pill bg-light text-dark me-2 mb-2 d-inline-flex align-items-center';
